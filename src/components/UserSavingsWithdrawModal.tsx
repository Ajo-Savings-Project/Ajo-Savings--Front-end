...
import { withdrawFromSavings } from "../slices/internalFundingSlice";
...
const UserSavingsWithdrawModal = ({ visible, onClose }: Prop) => {
  ...
  const handleSubmit = () => {
    try {
      ...
      dispatch(withdrawFromSavings(payload));
    } catch (error) {
      console.log(error);
    }
    console.log(amount);
    ...
    >
      <div className="relative w-[474px] h-[270px] bg-white px-[16px] pt-[24px] pb-[32px] flex flex-col justify-center items-center text-center rounded-lg ">
        <h1 className="text-[24px] text-[#101828] font-bold font-[Inter] font-not-italic my-[32px]">
          Withdraw from your Savings Wallet
        </h1>
...
export default UserSavingsWithdrawModal;
